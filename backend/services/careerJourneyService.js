...
async function updateStage(userId, skillKey, stage) {
  ...
  if (index > 0) {
    const previous = skill[STAGES[index - 1]];
    if (previous.status !== 'complete') {
      throw Object.assign(new Error(`Complete ${STAGES[index - 1]} before ${stage}.`), { statusCode: 409 });
    }
  }

  const next = { status: 'complete', completedAt: new Date() };
  const result = await CareerSkillJourney.findOneAndUpdate(
    { userId, 'skills.skillKey': skillKey },
    { $set: { [`skills.$.${stage}`]: next, [`skills.$.${STAGES[index + 1]}.status`]: index < 2 ? 'available' : 'locked', updatedAt: new Date() } },
    { new: true }
  ).lean();
  return result;
}
